...
def copy_cadence_fields_to_dynamic_cadence(apps, schema_editor):
    observation_groups = apps.get_model('bhtom_observations', 'ObservationGroup')
    for row in observation_groups.objects.exclude(cadence_strategy=''):
        dynamic_cadence = apps.get_model('bhtom_observations', 'DynamicCadence')
        try:
...
class Migration(migrations.Migration):
    dependencies = [
        ('bhtom_observations', '0009_observationrecord_user'),
    ]
    ...
    operations = [
        ...
        migrations.AddField(
            ...
            field=models.ForeignKey(on_delete=models.deletion.CASCADE, null=False, default=None,
                                    to='bhtom_observations.ObservationGroup'),
        ),

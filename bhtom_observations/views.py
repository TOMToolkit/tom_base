...
from bhtom_dataproducts.forms import AddProductToGroupForm, DataProductUploadForm
from bhtom_observations.cadence import CadenceForm, get_cadence_strategy
from bhtom_observations.facility import get_service_class, get_service_classes
from bhtom_observations.facility import BaseManualObservationFacility
from bhtom_observations.forms import AddExistingObservationForm
from bhtom_observations.models import ObservationRecord, ObservationGroup, ObservationTemplate, DynamicCadence
...
class ObservationListView(FilterView):
    ...
    template_name = 'bhtom_observations/observation_list.html'
    paginate_by = 25
    model = ObservationRecord
    strict = False
    ...
    def get_queryset(self, *args, **kwargs):
        ...
        if settings.TARGET_PERMISSIONS_ONLY:
            ...
            return get_objects_for_user(self.request.user, 'bhtom_observations.view_observationrecord')
...
class ObservationCreateView(LoginRequiredMixin, FormView):
    ...
    def form_valid(self, form):
        ...
        if len(records) > 1 or form.cleaned_data.get('cadence_strategy'):
            ...
            assign_perm('bhtom_observations.view_observationgroup', self.request.user, observation_group)
            assign_perm('bhtom_observations.change_observationgroup', self.request.user, observation_group)
            assign_perm('bhtom_observations.delete_observationgroup', self.request.user, observation_group)
        ...
        if not settings.TARGET_PERMISSIONS_ONLY:
            ...
            for record in records:
                assign_perm('bhtom_observations.view_observationrecord', groups, record)
                assign_perm('bhtom_observations.change_observationrecord', groups, record)
...
class AddExistingObservationView(LoginRequiredMixin, FormView):
    ...
    """
    template_name = 'bhtom_observations/existing_observation_confirm.html'
...
class ObservationGroupCreateView(LoginRequiredMixin, CreateView):
    ...
    def form_valid(self, form):
        ...
        obj.save()
        assign_perm('bhtom_observations.view_observationgroup', self.request.user, obj)
        assign_perm('bhtom_observations.change_observationgroup', self.request.user, obj)
        assign_perm('bhtom_observations.delete_observationgroup', self.request.user, obj)
...
class ObservationGroupListView(PermissionListMixin, ListView):
    ...
    """
    permission_required = 'bhtom_observations.view_observationgroup'
    model = ObservationGroup
    paginate_by = 25
...
class ObservationTemplateListView(FilterView):
    ...
    template_name = 'bhtom_observations/observationtemplate_list.html'
...
class ObservationTemplateCreateView(FormView):
    ...
    """
    template_name = 'bhtom_observations/observationtemplate_form.html'
...
class FacilityStatusView(TemplateView):
    template_name = 'bhtom_observations/facility_status.html'

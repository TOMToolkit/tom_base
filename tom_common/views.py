...
class UserUpdateView(LoginRequiredMixin, UpdateView):
    ...
    def form_valid(self, form):
        ...
        If the password was changed, updates the session auth hash to keep the
        user logged in (Django invalidates the session when the password hash
        changes). Encryption keys are independent of the password, so no

...
class LasairBrokerForm(GenericQueryForm):
    ...
    cone = forms.CharField(required=False, label='Object Cone Search',
        help_text='Object RA and Dec')
    sqlquery = forms.CharField(required=False, label='Freeform SQL query',
        help_text='SQL query')
...
def get_lasair_object(objectId):
    ...
    return {'alert_id': objectId,
        'timestamp': jdmax, 'ra':ra, 'dec':dec,
        'galactic_lng': glon, 'galactic_lat':glat, 'mag':magpsf}
...
class LasairBroker:
    ...
    def fetch_alerts(clazz, parameters):
        if 'cone' in parameters and len(parameters['cone'].strip()) > 0:
            response = requests.post(LASAIR_URL + '/conesearch/',
...
# note: the sql SELECT must include objectId
        if 'sqlquery' in parameters and len(parameters['sqlquery'].strip()) > 0:
            response = requests.post(LASAIR_URL + '/objlist/',

...
class TestMPCExplorerDataService(TestCase):
    ...
    def test_to_target(self):
        self.target_result['name'], self.target_result['aliases'] = self.ds.get_additional_mpc_names(
            self.target_result['mpc_orb'])
        ...
        orbit_data = self.target_result['mpc_orb']
    ...
    def test_to_target_no_name(self):
        ...
        self.target_result['mpc_orb']['designation_data']['iau_name'] = ""
        del (self.target_result['mpc_orb']['designation_data']['name'])
        self.target_result['mpc_orb']['designation_data']['orbfit_name'] = "2025AA"
        self.target_result['mpc_orb']['designation_data']['unpacked_primary_provisional_designation'] = \
            "2025 AA"
        self.target_result['mpc_orb']['designation_data']['unpacked_secondary_provisional_designations'] = []
        self.target_result['name'], self.target_result['aliases'] = self.ds.get_additional_mpc_names(
            self.target_result['mpc_orb'])
    ...
    def test_to_target_multiple_alternative_desigs(self):
        # Modify designation data to one with a provisional id only
        self.target_result['mpc_orb']['designation_data']['iau_name'] = ""
        self.target_result['mpc_orb']['designation_data']['name'] = 'Fringilla'
        self.target_result['mpc_orb']['designation_data']['orbfit_name'] = "709"
        self.target_result['mpc_orb']['designation_data']['unpacked_primary_provisional_designation'] = \
            "A911 CC"
        self.target_result['mpc_orb']['designation_data']['unpacked_secondary_provisional_designations'] = \
            [
    ...
    def test_comet_to_target(self):
        ...
        target_result['mpc_orb']['COM']['coefficient_values'][1] = 1.0
        target_result['mpc_orb']['categorization']['object_type_int'] = 10
        target_result['name'] = '65803'

        target = self.ds.to_target(target_result)
        self.assertEqual(target.type, 'NON_SIDEREAL')
        self.assertEqual(target.scheme, 'MPC_COMET')
        self.assertEqual(target.name, '65803')
        orbit_data = target_result['mpc_orb']
        elements = orbit_data['COM']['coefficient_values']
        self.assertEqual(target.epoch_of_elements, orbit_data['epoch_data']['epoch'])
        self.assertEqual(target.perihdist, elements[0])
        self.assertEqual(target.eccentricity, elements[1])
    ...
    def test_comet_to_target2(self):
        ...
        self.target_result['mpc_orb']['COM']['coefficient_values'][1] = 0.999
        self.target_result['mpc_orb']['categorization']['object_type_int'] = 11

...
When set to a number of days, API tokens older than that are rejected by ``tom_common.api_auth.TomTokenAuthentication``
...
regenerated by their owner after re-authenticating. Requires ``tom_common.api_auth.TomTokenAuthentication`` in

...
from bhtom_base.bhtom_targets.validators import RequiredFieldsTogetherValidator
...
class TargetSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        if group_serializer.is_valid():
            for group in groups:
                ...
                assign_perm('bhtom_targets.view_target', group_instance, target)
                assign_perm('bhtom_targets.change_target', group_instance, target)
                assign_perm('bhtom_targets.delete_target', group_instance, target)
    ...
    def update(self, instance, validated_data):
        ...
        if group_serializer.is_valid():
            for group in groups:
                group_instance = Group.objects.get(pk=group['id'])
                assign_perm('bhtom_targets.view_target', group_instance, instance)
                assign_perm('bhtom_targets.change_target', group_instance, instance)
                assign_perm('bhtom_targets.delete_target', group_instance, instance)  # TODO: add tests
...
class TargetFilteredPrimaryKeyRelatedField(serializers.PrimaryKeyRelatedField):
    ...
    def get_queryset(self):
        ...
        if not (request and queryset):
            return None
        return get_objects_for_user(request.user, 'bhtom_targets.change_target')

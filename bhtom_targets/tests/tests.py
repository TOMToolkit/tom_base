...
from bhtom_targets.models import Target, TargetExtra, TargetList, TargetName
from bhtom_targets.utils import import_targets
...
class TestTargetListUserPermissions(TestCase):
    def setUp(self):
        ...
        assign_perm('bhtom_targets.view_target', self.user, self.st1)
        assign_perm('bhtom_targets.view_target', self.user, self.st2)
        assign_perm('bhtom_targets.view_target', self.user2, self.st2)
...
class TestTargetListGroupPermissions(TestCase):
    def setUp(self):
        ...
        assign_perm('bhtom_targets.view_target', self.group1, self.st1)
        assign_perm('bhtom_targets.view_target', self.group2, self.st2)
...
class TestTargetDetail(TestCase):
    def setUp(self):
        ...
        assign_perm('bhtom_targets.view_target', user, self.st)
        assign_perm('bhtom_targets.view_target', user, self.nst)
...
class TestTargetNameSearch(TestCase):
    def setUp(self):
        ...
        assign_perm('bhtom_targets.view_target', self.user, self.st1)
        assign_perm('bhtom_targets.view_target', self.user2, self.st1)
        assign_perm('bhtom_targets.view_target', self.user, self.st2)
        assign_perm('bhtom_targets.view_target', self.user, self.st3)
...
class TestTargetGrouping(TestCase):
    ...
    def test_view_groupings(self):
        ...
        assign_perm('bhtom_targets.view_targetlist', user, group)
    ...
    def test_delete_group(self):
        ...
        user = User.objects.get(username='testuser')
        assign_perm('bhtom_targets.delete_targetlist', user, group)
...
class TestTargetAddRemoveGrouping(TestCase):
    def setUp(self):
        ...
        for _ in range(3):
            ...
            assign_perm('bhtom_targets.view_target', user, ft)
            assign_perm('bhtom_targets.change_target', user, ft)
        # create grouping
        self.fake_grouping = TargetGroupingFactory.create()
        assign_perm('bhtom_targets.view_targetlist', user, self.fake_grouping)

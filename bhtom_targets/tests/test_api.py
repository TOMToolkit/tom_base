...
class TestTargetViewset(APITestCase):
    def setUp(self):
        ...
        assign_perm('bhtom_targets.view_target', self.user, self.nst)
        assign_perm('bhtom_targets.add_target', self.user)
        assign_perm('bhtom_targets.change_target', self.user, self.st)
        assign_perm('bhtom_targets.change_target', self.user, self.nst)
        assign_perm('bhtom_targets.delete_target', self.user, self.st)

        # Create test user with subset of permissions
        self.user2 = User.objects.create(username='testuser2')
        assign_perm('bhtom_targets.view_target', self.user2, self.st)
    ...
    def test_target_create(self):
        ...
        self.assertEqual(get_objects_for_user(collaborator, 'bhtom_targets.view_target').first().name,
...
class TestTargetNameViewset(APITestCase):
    def setUp(self):
        ...
        assign_perm('bhtom_targets.delete_target', user, self.st)

...
from bhtom_base.bhtom_common.serializers import GroupSerializer
from bhtom_base.bhtom_dataproducts.models import DataProductGroup, DataProduct, ReducedDatum
from bhtom_base.bhtom_observations.models import ObservationRecord
from bhtom_base.bhtom_observations.serializers import ObservationRecordFilteredPrimaryKeyRelatedField
from bhtom_base.bhtom_targets.models import Target
from bhtom_base.bhtom_targets.serializers import TargetFilteredPrimaryKeyRelatedField
...
class DataProductSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        if group_serializer.is_valid() and settings.TARGET_PERMISSIONS_ONLY is False:
            for group in groups:
                ...
                assign_perm('bhtom_dataproducts.view_dataproduct', group_instance, dp)
                assign_perm('bhtom_dataproducts.change_dataproduct', group_instance, dp)
                assign_perm('bhtom_dataproducts.delete_dataproduct', group_instance, dp)
    ...
    def update(self, instance, validated_data):
        ...
        if group_serializer.is_valid() and not settings.TARGET_PERMISSIONS_ONLY:
            for group in groups:
                group_instance = Group.objects.get(pk=group['id'])
                assign_perm('bhtom_dataproducts.view_dataproduct', group_instance, instance)
                assign_perm('bhtom_dataproducts.change_dataproduct', group_instance, instance)
                assign_perm('bhtom_dataproducts.delete_dataproduct', group_instance, instance)

...
from bhtom_dataproducts.data_processor import run_data_processor
from bhtom_dataproducts.filters import DataProductFilter
from bhtom_dataproducts.models import DataProduct, ReducedDatum
from bhtom_dataproducts.serializers import DataProductSerializer
...
class DataProductViewSet(CreateModelMixin, DestroyModelMixin, ListModelMixin, GenericViewSet, PermissionListMixin):
    ...
    **Please note that ``groups`` are an accepted query parameters for the ``CREATE`` endpoint. The groups parameter
    ...
    permission_required = 'bhtom_dataproducts.view_dataproduct'
            ...
            try:
                ...
                if not settings.TARGET_PERMISSIONS_ONLY:
                    for group in response.data['group']:
                        assign_perm('bhtom_dataproducts.view_dataproduct', group, dp)
                        assign_perm('bhtom_dataproducts.delete_dataproduct', group, dp)
                        assign_perm('bhtom_dataproducts.view_reduceddatum', group, reduced_data)
            except Exception:
    ...
    def get_queryset(self):
        ...
        if settings.TARGET_PERMISSIONS_ONLY:
            ...
            return get_objects_for_user(self.request.user, 'bhtom_dataproducts.view_dataproduct')

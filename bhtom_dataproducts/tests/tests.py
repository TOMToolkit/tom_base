...
from bhtom_base.bhtom_dataproducts.models import DataProduct, is_fits_image_file
from bhtom_base.bhtom_dataproducts.processors.data_serializers import SpectrumSerializer
from bhtom_base.bhtom_dataproducts.processors.photometry_processor import PhotometryProcessor
from bhtom_base.bhtom_dataproducts.processors.spectroscopy_processor import SpectroscopyProcessor
from bhtom_base.bhtom_dataproducts.utils import create_image_dataproduct
from bhtom_base.bhtom_observations.tests.utils import FakeRoboticFacility
from bhtom_base.bhtom_observations.tests.factories import SiderealTargetFactory, ObservingRecordFactory
...
class Views(TestCase):
    def setUp(self):
        ...
        assign_perm('bhtom_targets.view_target', user, self.target)
    ...
    @patch('bhtom_base.bhtom_dataproducts.models.fits_to_jpg', mock_fits2image)
    @patch('bhtom_base.bhtom_dataproducts.models.find_fits_img_size', mock_find_fits_img_size)
    @patch('bhtom_base.bhtom_dataproducts.models.is_fits_image_file', mock_is_fits_image_file)
...
class TestViewsWithPermissions(TestCase):
    def setUp(self):
        ...
        assign_perm('bhtom_targets.view_target', self.user, self.target)
        assign_perm('bhtom_targets.view_target', self.user2, self.target)
        assign_perm('bhtom_targets.view_dataproduct', self.user, self.data_product)
...
@override_settings(TOM_FACILITY_CLASSES=['bhtom_base.bhtom_observations.tests.utils.FakeRoboticFacility'],
                   TARGET_PERMISSIONS_ONLY=True)
@patch('bhtom_base.bhtom_dataproducts.views.run_data_processor')
...
class TestDeleteDataProducts(TestCase):
    def setUp(self):
        ...
        assign_perm('bhtom_targets.view_target', self.user2, self.target)
        assign_perm('bhtom_targets.view_dataproduct', self.user, self.data_product)
        assign_perm('bhtom_targets.view_dataproduct', self.user2, self.data_product)
        assign_perm('bhtom_targets.delete_dataproduct', self.user, self.data_product)
        self.client.force_login(self.user)
...
class TestDataProcessor(TestCase):
    ...
    @patch('bhtom_base.bhtom_dataproducts.processors.spectroscopy_processor.SpectroscopyProcessor._process_spectrum_from_fits',
           return_value=('', ''))
    @patch('bhtom_base.bhtom_dataproducts.processors.spectroscopy_processor.SpectrumSerializer.serialize', return_value={})
    ...
    @patch('bhtom_base.bhtom_dataproducts.processors.spectroscopy_processor.SpectroscopyProcessor._process_spectrum_from_plaintext',
           return_value=('', ''))
    @patch('bhtom_base.bhtom_dataproducts.processors.spectroscopy_processor.SpectrumSerializer.serialize', return_value={})
    ...
    def test_process_spectrum_from_fits(self):
        with open('bhtom_base/bhtom_dataproducts/tests/test_data/test_spectrum.fits', 'rb') as spectrum_file:
    ...
    def test_process_spectrum_from_plaintext(self):
        with open('bhtom_base/bhtom_dataproducts/tests/test_data/test_spectrum.csv', 'rb') as spectrum_file:
    ...
    @patch('bhtom_base.bhtom_dataproducts.processors.photometry_processor.PhotometryProcessor._process_photometry_from_plaintext')
...
class TestDataProductModel(TestCase):
    ...
    def test_create_thumbnail(self, mock_is_fits_image_file):
        ...
        mock_is_fits_image_file.return_value = True
        with self.assertLogs('bhtom_base.bhtom_dataproducts.models', level='WARN') as logs:
            self.data_product.create_thumbnail()
            expected = ('WARNING:bhtom_base.bhtom_dataproducts.models:Unable to create thumbnail '

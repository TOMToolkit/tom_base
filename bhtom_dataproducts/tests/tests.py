...
from bhtom_dataproducts.models import DataProduct, is_fits_image_file
from bhtom_dataproducts.processors.data_serializers import SpectrumSerializer
from bhtom_dataproducts.processors.photometry_processor import PhotometryProcessor
from bhtom_dataproducts.processors.spectroscopy_processor import SpectroscopyProcessor
from bhtom_dataproducts.utils import create_image_dataproduct
from bhtom_observations.tests.utils import FakeRoboticFacility
from bhtom_observations.tests.factories import SiderealTargetFactory, ObservingRecordFactory
...
@patch('bhtom_dataproducts.models.DataProduct.get_preview', return_value='/no-image.jpg')
class Views(TestCase):
    ...
    @patch('bhtom_dataproducts.models.fits_to_jpg', mock_fits2image)
    @patch('bhtom_dataproducts.models.find_fits_img_size', mock_find_fits_img_size)
    @patch('bhtom_dataproducts.models.is_fits_image_file', mock_is_fits_image_file)
...
class TestDataProductListView(TestCase):
    ...
    @patch('bhtom_dataproducts.models.is_fits_image_file')
...
@override_settings(TOM_FACILITY_CLASSES=['bhtom_observations.tests.utils.FakeRoboticFacility'],
                   TARGET_PERMISSIONS_ONLY=True)
@patch('bhtom_dataproducts.views.run_data_processor')
...
class TestDataProcessor(TestCase):
    ...
    @patch('bhtom_dataproducts.processors.spectroscopy_processor.SpectroscopyProcessor._process_spectrum_from_fits',
           return_value=('', ''))
    @patch('bhtom_dataproducts.processors.spectroscopy_processor.SpectrumSerializer.serialize', return_value={})
    ...
    @patch('bhtom_dataproducts.processors.spectroscopy_processor.SpectroscopyProcessor._process_spectrum_from_plaintext',
           return_value=('', ''))
    @patch('bhtom_dataproducts.processors.spectroscopy_processor.SpectrumSerializer.serialize', return_value={})
    ...
    @patch('bhtom_dataproducts.processors.photometry_processor.PhotometryProcessor._process_photometry_from_plaintext')
...
class TestDataProductModel(TestCase):
    ...
    def test_create_thumbnail(self, mock_is_fits_image_file):
        ...
        mock_is_fits_image_file.return_value = True
        with self.assertLogs('bhtom_dataproducts.models', level='WARN') as logs:
            self.data_product.create_thumbnail()
            expected = ('WARNING:bhtom_dataproducts.models:Unable to create thumbnail '
